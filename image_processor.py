...
def rotate(image, angle, top_point, nipple_point):
    ...
    return img_translation, nipple_point, translation_matrix
...
def change_image_size(new_name,image, path, index, list_of_sizes, x_max, y_max):
    ...
    cv2.imwrite(path + "\\" + new_name, blank_image)
...
def draw_roi(full_path,p_image,p_mask, new_name):
    img = cv2.imread(full_path + "\\"+p_mask, cv2.THRESH_BINARY)
    img_full = cv2.imread(full_path + "\\"+p_image)
    ...
    cv2.rectangle(img_full, (x, y), (x + w, y + h), (200, 0, 0), 10)
    cv2.imwrite(full_path + "\\" + new_name, img_full)
...
def run_processing(source, tagged, path, mask):
    ...
    if angle < 90:
        ...
        source_rotated, nipple_point, translation_matrix = rotate(source, angle, top_point, nipple_point)
        num_rows, num_cols = source_rotated.shape[:2]
    ...
    cropped_mask = crop_breast_from_image(int(width_breast), int(y_top), int(y_bottom), mask)

    cv2.imwrite(path+"\\"+"source.png",cropped_image)
    cv2.imwrite(path + "\\" + "mask_cropped.png", cropped_mask)
    draw_roi(path, "source.png", "mask_cropped.png", "roi.png")
